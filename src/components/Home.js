...
const Home = () => {
  ...
  const randomHeroImage = mockData[Math.floor(Math.random() * 30)].fields.image;

  // Display 10 destinations from seed data:
  const shuffledDestinations = mockData.sort(() => 0.5 - Math.random());
  const destinationSubset = shuffledDestinations.slice(0, 10);
